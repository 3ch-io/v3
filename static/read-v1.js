	let readhtml = "read.html ver 02.0e - 2023/06/17";
	let spcheck = '';
	...
	let imglist = [];
	let plist = [];
	if (ls && ls.indexOf('l') == -1 && ls.indexOf('n') == -1) areload = false;
if (isSmartPhone() == true && location.host.indexOf('rentalbbs.net') != -1) {
...
document.getElementById('body').innerHTML += '<link href="https://rentalbbs.net/css/lightbox.css" rel="stylesheet"><style id="mute"></style><style id="prev-hide"></style>';
if (isSmartPhone() == false) document.getElementById('body').innerHTML += '<style>body{color:#697699 !important;}a{color:#485269 !important;}.thread,.title{padding-left:15px;padding-right:15px}.post,.message{padding:10px 0}.number,.name,.date,.ids{font-size:12px;color:#697699 !important;margin-right:5px;padding-left:0;margin-left:0}.thread{width: 75%;}.side{display: block;border: 0px solid #F33; position: fixed;    right: 0em; top:   0em; bottom: auto; width: 25%; height: 100%; z-index:  1; margin: 0; padding: 0; color: #333; background: #FFF; overflow-y: scroll;padding-top: 8em;}#headline{height: auto;overflow-y: auto;border: none;}</style>';
...
let mutes = '';
if (areload != "true") {
	if (document.getElementsByClassName('newpostbutton')[0]) document.getElementsByClassName('newpostbutton')[0].style.display = 'none';
	if (document.getElementsByClassName('newposts')[0]) document.getElementsByClassName('newposts')[0].innerHTML += '<center><a href="javascript:reload()">新着チェック</a></center>';
}
			//スレッド描画
			let threaddata = document.getElementsByClassName('thread')[0];
			let data = threaddata.innerHTML.split('\n');
			...
			if (!getprev && !document.getElementById(2) && number > 2) getprev = number;
			let numtext = '<a href="javascript:Menu('+number+')" class="number" id="number-'+number+'">'+number+'</a>';
			let nametext = '<span class="name" id="name-'+number+'" style="'+document.getElementById('name-'+number).style.cssText+'">'+document.getElementById('name-'+number).innerHTML+'</span>';
			let datetext = '<span class="date" id="date-'+number+'">'+document.getElementById('date-'+number).innerHTML+'</span>';
			let msgtext = '<div class="message" id="msg-'+number+'">'+document.getElementById('msg-'+number).innerHTML+'</div>';
			if (isSmartPhone() == true) document.getElementById(number).innerHTML = numtext+nametext+msgtext+datetext+'<div id="replys-'+number+'" style="display:none;"><img class="aresicon" src="//rentalbbs.net/static/ares.svg" width="12" height="12"><small id="rcount-'+number+'" class="rcount"></small> :<span id="back-'+number+'"></span></div>';
			else document.getElementById(number).innerHTML = numtext+nametext+datetext+msgtext+'<div id="replys-'+number+'" style="display:none;"><img class="aresicon" src="//rentalbbs.net/static/ares.svg" width="12" height="12"><small id="rcount-'+number+'" class="rcount"></small> :<span id="back-'+number+'"></span></div>';
			let NG;
			let ntext = document.getElementById('name-'+number).innerHTML;
			let NAME = ntext.match(/<b>(.*)<\/b>/);
			let from;
			if (NAME) from = NAME[1];
			from = from.replace('</a>', '');
			let names = from.split('</b>');
			let name = names[0];
			name = name.replace('<b>', '');
			let id2 = names[1];
			let id3 = names[2];
			...
			let mailto = ntext.match(/<a href="mailto:(.+)"><b>/);
			let mail;
			if (ntext.indexOf('mailto:') != -1) mail = mailto[1];
			if (id2) name += ' '+id2;
			if (id3) name += ' '+id3;
			if (id4) name += ' '+id4;
			if (mail) name += '['+mail+']';
			document.getElementById('name-'+number).innerHTML = name;
			let dateid = document.getElementById('date-'+number).innerHTML;
			let dateids = dateid.split(' ');
			...
			if (isSmartPhone() == true) document.getElementById('date-'+number).innerHTML = ids+date;
			else document.getElementById('date-'+number).innerHTML = date+ids;
			let message = document.getElementById('msg-'+number).innerHTML;
			let rnum = message.match(/&gt;&gt;([0-9]+)(?![-\d])/);
			if (rnum) rnum = rnum[1];
			if (rnum != number && rnum != 1 && rnum < 1000 && document.getElementById(rnum) && tshide != "false" && treeView != "false") document.getElementById(number).style.display = 'none';
			if (ghide == "auto") {
...
if (areload != false) localStorage.setItem(bbs+key, LINENUM);
...
function Getresponse() {
 let loggd = '';
 if (!getCookie("email")) loggd = '<br><a href="https://rentalbbs.net/login.html">ログインして書くことができます</a>';
...
if (location.host.indexOf('rentalbbs.net') != -1) {
...
function reload(st) {
	...
	request.open('GET', '/test/read.html/' + bbs + '/' + key + '/' + st + '-n');
	...
	request.onreadystatechange = () => {
		if(request.readyState === 4 && request.status === 200) {
			...
			if (isSmartPhone() == true) newpost.innerHTML = numtext+nametext+msgtext+datetext+'<div id="replys-'+number+'" style="display:none;"><img class="aresicon" src="//rentalbbs.net/static/ares.svg" width="12" height="12"><small id="rcount-'+number+'" class="rcount"></small> :<span id="back-'+number+'"></span></div>';
			else newpost.innerHTML = numtext+nametext+datetext+msgtext+'<div id="replys-'+number+'" style="display:none;"><img class="aresicon" src="//rentalbbs.net/static/ares.svg" width="12" height="12"><small id="rcount-'+number+'" class="rcount"></small> :<span id="back-'+number+'"></span></div>';
			let mae = number;
			mae--;
...
return;
	...
	notice();
	let request = new XMLHttpRequest();
	request.open('GET', '/test/read.html/' + bbs + '/' + key + '/2-'+to+'n');
	request.timeout = 3 * 1000;
	request.send();
	request.onreadystatechange = () => {
		if(request.readyState === 4 && request.status === 200) {
...
function ID(Id, n, num) {
			if (isSmartPhone() == true) did = Id.replace(/ID:/gi, '');
			else did = Id;
			let idr = idlist[did];
			let idcount;
...
if (val.indexOf('Copyr-') != -1) {
  ...
  if (navigator.clipboard) {
	...
	let date = d2.replace(/\n/, '');
	if (id.indexOf('ID:') == -1 && id.length > 5) id = 'ID:'+id;
...
if (val == "setting") {
let treecheck,tscheck,arecheck,gurocheck,autocheck,gcheck,rcheck,darkcheck;
if (treeView == "true") treecheck = 'checked';
if (tshide == "true") tscheck = 'checked';
...
document.getElementById('modal_text').innerHTML = '<div id="optionInsideView" class="option_container_view"><div class="option_style_1"><div class="option_style_2">閲覧設定</div><div class="option_style_3">&emsp;</div><div class="option_style_4"><div class="option_style_5"><input class="option_style_6" '+spcheck+' id="spmode" type="checkbox">スマホ版</div><div class="option_style_5"><input class="option_style_6" '+darkcheck+' id="darkmode" type="checkbox">ダークモード</div><div class="option_style_5"><input class="option_style_6" '+treecheck+' id="tView" name="auto_post" type="checkbox">ツリー表示</div><div class="option_style_5"><input class="option_style_6" '+tscheck+' id="ts_hide" type="checkbox">ツリー表示で重複したレスを非表示</div><div class="option_style_5"><input class="option_style_6" '+arecheck+' id="arecheck" type="checkbox">5秒間隔で自動更新する</div><div class="option_style_5"><input class="option_style_6" '+autocheck+' id="autoscroll" type="checkbox">新着投稿の位置まで自動スクロール</div><div class="option_style_5"><input class="option_style_6" '+gurocheck+' id="g_hide" type="checkbox">注意のある画像を自動的に非表示</div><div class="option_style_5"><input class="option_style_6" '+gcheck+' id="a_hide" type="checkbox">画像のサムネイル表示をオフにする</div><div class="option_style_5"><input class="option_style_6" '+rcheck+' id="usereadcgi" type="checkbox">常にread.cgiで表示(クラシック版)</div></div><div class="option_style_11"><button id="saveOptions" class="option_style_12" onclick="setoption()">変更を保存</button><button id="cancelOptions" class="option_style_13" onclick="rclose()">キャンセル</button></div></div></div>';
document.getElementById('rModal').style.display = 'block';
return;
}
if (val == "ngword") {
...
function setoption() {
	let tView,tshide,ghide,readcgi,ngword;
	if (document.getElementById('tView').checked == true) tView = true;
	else tView = false;
	localStorage.setItem('treeView', tView);
	if (document.getElementById('ts_hide').checked == true) tshide = true;
	else tshide = false;
	localStorage.setItem('tshide', tshide);
...
mutes += '.tv-'+number+'{display:none !important;}';
document.getElementById('mute').innerHTML = '<style>'+mutes+'</style>';
}

function imghide(n) {
mutes += '.img-'+n+'{display:none !important;}';
document.getElementById('mute').innerHTML = '<style>'+mutes+'</style>';
}
